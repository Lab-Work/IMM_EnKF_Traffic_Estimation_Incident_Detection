...
def Adjust(previousMode, ModelNumber, pTran):
    adjustFactor=ones(ModelNumber)*(1-pTran)/(ModelNumber-1)
    adjustFactor[0]=pTran
    for i in range(2,len(previousMode)-2):
        if previousMode[i]==1:
            adjustFactor[0]=(1-pTran)/(ModelNumber-1)
            adjustFactor[1+2*(i-2)]=pTran
        elif previousMode[i]==2:
            adjustFactor[0]=(1-pTran)/(ModelNumber-1)
            adjustFactor[2+2*(i-2)]=pTran
    return adjustFactor
...
for i in range(int(ModelNumber)):
    if i==0:
        ...
        PModel[i]=(1-pTran)/(ModelNumber-1)
...
# model inference    
    adjustFactor = Adjust(EstimatedMode[counter-1], ModelNumber, pTran)
    ModelProbSum=sum(ModelProb*adjustFactor)
    ModelProb=ModelProb*adjustFactor/ModelProbSum
    print ModelProb
...
plt.savefig('ModelProb'+str(GPS.PR)+'.pdf',bbox_inches='tight')
plt.hold(False)
plt.show()
plt.clf()
